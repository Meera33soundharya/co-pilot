...
export default function Dashboard() {
    ...
    const highPri = complaints.filter(c => c.priority === "High" && c.status !== "Closed" && c.status !== "Resolved" && c.status !== "Rejected").length;
    ...
    function generateBriefing() {
        ...
        setTimeout(() => {
            const topCat = ["Roads", "Water", "Electricity", "Sanitation"].find(c =>
                complaints.some(x => x.category === c && x.status === "Pending")) ?? "Roads";
    ...
    const statusPie = (["Pending", "Accepted", "Rejected", "Assigned", "In Progress", "Resolved", "Closed"] as Status[])
        .map(s => ({ name: s, value: complaints.filter(c => c.status === s).length, fill: STATUS_COLOR[s] }))
        .filter(d => d.value > 0);

    // Urgent queue — priority sorted, not resolved/closed/rejected
                                        ...
                                        <div className="flex flex-col gap-1.5 shrink-0">
                                            {isAdmin && c.status === "Pending" && (
                                                <div className="flex gap-1.5">
                                                    <button onClick={e => { e.stopPropagation(); updateStatus(c.id, "Accepted", "Complaint accepted by Admin"); }}
                                                        className="flex items-center gap-1.5 px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[10px] font-black transition-all shadow-sm">
                                                        Accept
                                                    </button>
                                                    <button onClick={e => { e.stopPropagation(); updateStatus(c.id, "Rejected", "Complaint rejected by Admin"); }}
                                                        className="flex items-center gap-1.5 px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-xl text-[10px] font-black transition-all shadow-sm">
                                                        Reject
                                                    </button>
                                                </div>
                                            )}
                                            {(isAdmin || isOfficer) && (c.status === "Assigned" || c.status === "Accepted") && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white rounded-3xl border border-gray-100 p-7 shadow-sm">
                        ...
                        <div className="space-y-2.5">
                            {(["Pending", "Accepted", "Assigned", "In Progress"] as Status[]).map(s => {

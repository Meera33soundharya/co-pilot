...
export default function DashboardLayout({ children, title, subtitle, bgImage, actions, isDark = false }: DashboardLayoutProps) {
    ...
    return (
        <div className="flex h-screen overflow-hidden" style={{ backgroundColor: isDark ? "#0A0F1C" : "#F9FAFB", fontFamily: "'Inter', sans-serif" }}>
            ...
            {/* ── Sidebar ──────────────────────────────────────────── */}
            <aside
            className={`fixed lg:relative inset-y-0 left-0 z-[130] lg:z-[5] flex flex-col w-[260px] shrink-0 border-r border-gray-200 transform transition-transform duration-300 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
            ...
            >
                ...
                {/* Role strip */}
                <div className="mx-3 mt-3 px-4 py-2.5 rounded-xl border flex items-center gap-2 font-bold" style={{
                ...
                <span className="truncate text-sm font-bold">{currentUser?.name ?? roleCfg.label}</span>
                </div>
                ...
                {(role === "admin" || role === "officer") && newCount > 0 && (
                    <button
                        ...
                        className="mx-3 mt-2 flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold uppercase tracking-wide text-red-700 bg-red-50 border border-red-100 hover:bg-red-100 transition-colors group"
                    >
                ...
                <div className="px-3 py-3 border-t border-gray-100">
                    <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-all group border border-transparent hover:border-gray-100"
                    >
                        <div
                        ...
                        <div className="flex-1 text-left min-w-0">
                            <p className="text-sm font-bold text-gray-900 truncate leading-tight">{currentUser?.name ?? roleCfg.label}</p>
                            <p className="text-xs font-semibold text-gray-400 capitalize mt-0.5 uppercase tracking-wide">{role}</p>
                        </div>
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
                        ...
                        {role !== "citizen" ? (
                            <div className="relative hidden md:block">
                                ...
                                <input
                                    ...
                                    className="pl-9 pr-4 py-1.5 bg-gray-50 border border-gray-200 rounded-lg text-sm w-56 focus:outline-none focus:border-[#B91C1C]/30 focus:ring-2 focus:ring-[#B91C1C]/10 font-medium text-gray-700 placeholder:text-gray-400 transition-all"
                                />
                            </div>
                        ) : (
                ...
                <div className={`flex-1 overflow-y-auto scroll-smooth relative ${isDark ? "bg-[#0A0F1C]" : "bg-[#F9FAFB]"}`}>
                    ...
                    {isDark && (
                        <div className="fixed top-0 left-[260px] right-0 bottom-0 z-0 pointer-events-none opacity-40 overflow-hidden">
                            <img
                    ...
                    <div className="relative z-10 p-4 lg:p-6 max-w-[1800px] mx-auto">

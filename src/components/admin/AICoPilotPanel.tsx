    ...
    <div className="flex flex-col h-full overflow-hidden rounded-[2rem] border border-white/10 bg-[#060912]/80 backdrop-blur-2xl shadow-2xl shadow-black/50">
      ...
      <div className="px-6 py-4 border-b border-white/10 bg-white/5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-bold text-white tracking-wide">Analysis Engine</span>
        </div>
        <div className="flex gap-2">
          <button className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs font-bold text-white/70 hover:text-white hover:bg-white/10 transition-colors">Clear</button>
          <button className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs font-bold text-white/70 hover:text-white hover:bg-white/10 transition-colors flex items-center gap-1">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar relative">
        ...
        <div className="space-y-4">
          ...
          <div className="relative group">
            <textarea 
              ...
              className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white/90 text-sm font-medium placeholder:text-white/30 focus:outline-none focus:border-[#C81D25]/50 focus:ring-1 focus:ring-[#C81D25]/50 transition-all min-h-[160px] resize-y custom-scrollbar"
            />
            ...
            <div className="absolute inset-0 border-2 border-dashed border-[#C81D25]/0 rounded-2xl pointer-events-none group-hover:border-[#C81D25]/30 transition-colors flex items-center justify-center">
              <div className="opacity-0 group-hover:opacity-100 flex flex-col items-center gap-2 text-white/40 transition-opacity bg-[#060912]/80 px-6 py-4 rounded-xl backdrop-blur-md">
                ...
                <span className="text-xs font-bold tracking-widest uppercase">Drop Files</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button 
              disabled={loading} 
              onClick={runAnalyze} 
              className="relative overflow-hidden group px-6 py-3 bg-gradient-to-r from-[#C81D25] to-[#800F15] text-white rounded-xl font-bold text-sm shadow-lg shadow-[#C81D25]/20 hover:shadow-[#C81D25]/40 transition-all active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex items-center gap-2"
            >
            ...
            <button disabled className="px-5 py-3 bg-white/5 border border-white/10 text-white/70 hover:text-white hover:bg-white/10 rounded-xl font-bold text-sm transition-all disabled:opacity-50 flex items-center gap-2">
        ...
        {(streaming || result) && (
          <div className="mt-8 pt-8 border-t border-white/10 animate-fade-in">
            ...
            {streaming && !result ? (
              <div className="bg-black/20 rounded-2xl p-6 border border-white/5 font-mono text-sm text-emerald-400 whitespace-pre-wrap leading-loose shadow-inner">
                {streamText}
              ...
              <div className="space-y-4 animate-slide-up">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-2 relative overflow-hidden">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/40 flex items-center gap-1.5"><BarChart className="w-3 h-3" /> Category</span>
                    <span className="text-base font-bold text-white truncate">{result.category}</span>
                  </div>
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-2 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-1 h-full bg-amber-500" />
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/40 flex items-center gap-1.5"><ShieldAlert className="w-3 h-3" /> Priority</span>
                    <span className="text-base font-bold text-white truncate">{result.priority}</span>
                  </div>
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-2 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-1 h-full bg-emerald-500" />
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/40 flex items-center gap-1.5"><CheckCircle2 className="w-3 h-3" /> Confidence</span>
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                  <h4 className="text-xs font-black uppercase tracking-widest text-white/40 mb-3">Executive Summary</h4>
                ...
                {result.actionPlan && result.actionPlan.length > 0 && (
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                    <h4 className="text-xs font-black uppercase tracking-widest text-white/40 mb-4">Recommended Action Plan</h4>
                    <ul className="space-y-3">
                      {result.actionPlan.map((step: string, i: number) => (
                        <li key={i} className="flex gap-3 text-sm text-white/80 items-start">
                          <span className="flex-shrink-0 w-6 h-6 rounded-full bg-[#C81D25]/20 text-[#C81D25] flex items-center justify-center font-black text-[10px] border border-[#C81D25]/30">
                            {i + 1}
                          </span>
                ...
                {result.reasoning && (
                  <div className="bg-black/30 border border-white/5 rounded-2xl p-6 shadow-inner">
                    <h4 className="text-xs font-black uppercase tracking-widest text-white/30 mb-3 flex items-center gap-2">
                      <Bot className="w-3 h-3" /> System Reasoning Log
                    </h4>
                    <p className="text-xs text-white/50 leading-relaxed font-mono whitespace-pre-wrap">{result.reasoning}</p>
                  </div>
                )}
              </div>
            ) : null}
        ...
        <div className="p-4 border-t border-white/10 bg-white/5">
          <div className="relative">
            <input 
              ...
              className="w-full bg-[#060912]/50 border border-white/10 rounded-xl pl-4 pr-12 py-3 text-sm text-white focus:outline-none focus:border-[#C81D25]/50 focus:ring-1 focus:ring-[#C81D25]/50 transition-all placeholder:text-white/30"
            />

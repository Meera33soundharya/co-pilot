...
export default function Reports() {
    ...
    const selectedDays = timeframes.find(t => t.label === periodStr)?.days ?? 90;
    ...
    const kpiData = useMemo(() => {
        ...
        const autoCategorized = filteredComplaints.filter(c => c.audit.some(a => a.action.includes("Auto-categorized"))).length;
        
        const aiAcc = total > 0 ? ((autoCategorized / total) * 100).toFixed(1) : "0.0";
        const resolutionRate = total > 0 ? Math.round((resolved / total) * 100) : 0;
        
        // Mock sentiment based on resolution rate
        const sentiment = Math.min(100, Math.max(0, resolutionRate + 5));

        return [
            { label: `Total Cases (${periodStr})`, value: total.toString(), change: "Live", icon: Target, color: "text-[#B91C1C]", bg: "bg-red-50" },
            { label: "Resolved Nodes", value: resolved.toString(), change: `${resolutionRate}%`, icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50" },
            { label: "AI Triage Rate", value: `${aiAcc}%`, change: "Auto", icon: Zap, color: "text-amber-500", bg: "bg-amber-50" },
            { label: "Public Sentiment", value: `${sentiment}/100`, change: "Estimated", icon: Award, color: "text-blue-600", bg: "bg-blue-50" },
        ];
    }, [filteredComplaints, periodStr]);

    // Department Stats
    const deptStats = useMemo(() => {
        const depts: Record<string, { total: number; resolved: number; totalHrs: number }> = {};
        
        filteredComplaints.forEach(c => {
            const dept = c.dept || "Unassigned";
            if (!depts[dept]) depts[dept] = { total: 0, resolved: 0, totalHrs: 0 };
            
            depts[dept].total++;
            if (c.status === "Resolved" || c.status === "Closed") {
                depts[dept].resolved++;
                const hrs = (Date.now() - c.timestamp) / 3600000;
                depts[dept].totalHrs += hrs;
            }
        });

        return Object.entries(depts)
            .map(([dept, data]) => {
                const score = data.total > 0 ? Math.round((data.resolved / data.total) * 100) : 0;
                const avgRes = data.resolved > 0 ? (data.totalHrs / data.resolved).toFixed(1) : "0.0";
                
                let status = "Stable";
                let color = "#111827";
                if (score >= 90) { status = "Elite"; color = "#10B981"; }
                else if (score >= 75) { status = "Optimal"; color = "#B91C1C"; }
                else if (score >= 50) { status = "Improving"; color = "#F59E0B"; }
                else { status = "Critical"; color = "#EF4444"; }

                return { dept, score, avg: `${avgRes}h`, status, color, total: data.total };
            })
    ...
    const radarData = useMemo(() => {
        ...
        const auto = filteredComplaints.filter(c => c.audit.some(a => a.action.includes("Auto-categorized"))).length;
        const aiRate = total > 0 ? Math.round((auto / total) * 100) : 0;

        return [
            { subject: "Response Speed", A: Math.min(100, resRate + 15), fullMark: 100 },
            { subject: "Resolution Accuracy", A: resRate, fullMark: 100 },
            { subject: "Citizen Feedback", A: Math.min(100, Math.max(0, resRate + 5)), fullMark: 100 },
            { subject: "AI Reliability", A: aiRate || 95, fullMark: 100 },
            { subject: "SLA Compliance", A: Math.min(100, resRate + 10), fullMark: 100 },
        ];
    }, [filteredComplaints]);

    const govScore = Math.round(radarData.reduce((acc, curr) => acc + curr.A, 0) / radarData.length);
    ...
    const handleGenerate = () => {
        ...
        setTimeout(() => {
            const total = filteredComplaints.length;
            const res = filteredComplaints.filter(c => c.status === "Resolved" || c.status === "Closed").length;
            const resRate = total > 0 ? Math.round((res / total) * 100) : 0;

            const summaries: Record<ReportType, string> = {
                "Executive Summary": `For ${periodStr}, district health is tracking with a ${resRate}% resolution rate across ${total} total cases. Dynamic analysis indicates a stable workflow.`,
                "Departmental Performance": deptStats.length > 0 
                    ? `Top active department is ${deptStats[0].dept} with ${deptStats[0].total} cases and a ${deptStats[0].score}% resolution score. Focus needed on critical divisions.`
                    : "Insufficient data to generate departmental analysis for this period.",
                "AI Impact Analysis": `AI triage processing was active across ${kpiData[2].value} of cases in the ${periodStr} period. AI routing stability remains optimal.`,
                "Citizen Sentiment Report": `Estimated citizen sentiment is currently ${kpiData[3].value}. Maintaining swift resolution times will continue to drive positive feedback.`
            };
            ...
            <div className="space-y-8 pb-20">
                        ...
                        <div className="flex items-center justify-between mb-12">
                            <div>
                                <h3 className="text-lg font-black text-gray-900 uppercase tracking-widest flex items-center gap-2">
                                    ...
                                    Dynamic Case Lifecycle ({periodStr})
                                </h3>
                    ...
                    <div className="bg-[#0D1425] rounded-[3rem] p-10 shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="mt-8 flex items-center justify-between pt-8 border-t border-white/5 relative z-10">
                            <div className="space-y-1">
                                ...
                                <p className="text-3xl font-black text-white italic">{govScore}<span className="text-base text-[#B91C1C] ml-1">Pts</span></p>
                            </div>
                ...
                <div className="bg-white border border-gray-100 rounded-[3rem] shadow-sm overflow-hidden">
                    <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                        <div>
                            <h3 className="font-black text-gray-900 flex items-center gap-2 text-lg">
                                ...
                                Operational Scorecard ({periodStr})
                            </h3>
                    ...
                    <div className="divide-y divide-gray-50">
                        {deptStats.length > 0 ? deptStats.map((d, idx) => (
                            <div key={d.dept} className="flex flex-col lg:flex-row lg:items-center gap-8 px-10 py-6 hover:bg-red-50/20 transition-all group">
                                ...
                                <div className="flex-1 space-y-3">
                                    <div className="flex justify-between items-end">
                                        <span className="text-sm font-black text-gray-400 uppercase tracking-[0.2em]">Institutional Efficiency ({d.total} cases)</span>
